    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.info(request, _('Registration successful! Please check your email to verify your account.'))
            except Exception as e:
                logger.error(f"Failed to send verification email: {e}", exc_info=True)
                messages.warning(request, _('Account created, but failed to send verification email.'))
...
class CustomLoginView(LoginView):
    ...
    def form_valid(self, form):
        ...
        # Periksa verifikasi email (kecuali untuk admin)
...
def verify_email(request, token):
    ...
    try:
        ...
        print("Verifikasi berhasil, mengarahkan ke halaman sukses")
        return render(request, 'accounts/verification_success.html', {
            'title': 'Verifikasi Berhasil',
            'message': 'Email Anda berhasil diverifikasi!',
            'redirect_url': reverse_lazy('manager_dashboard:dashboard'),
            'redirect_text': 'Ke Dashboard'
        })
